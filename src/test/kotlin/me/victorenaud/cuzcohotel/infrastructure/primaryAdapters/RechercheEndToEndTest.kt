...
internal class RechercheEndToEndTest {
    ...
    fun `lors d'une recherche renvoie toutes les chambres disponibles`() {
        // When
        mvc.perform(MockMvcRequestBuilders.get("/api/v0/recherche?invités=1"))

            // Then
            .andExpect(MockMvcResultMatchers.status().isOk)
            .andExpect(jsonPath<List<Any>>("$", hasSize(12)))
            .andExpect(jsonPath("$[0].étage", Is("1")))
            .andExpect(jsonPath("$[0].numéro", Is("101")))
            .andExpect(jsonPath("$[0].capacité", Is(2)))
            .andExpect(
                jsonPath(
                    "$[0].description",
